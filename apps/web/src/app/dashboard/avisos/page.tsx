...
import { avisosService, Aviso, TipoAviso } from '@/services/aviso-service';
...
export default function AvisosPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      console.error(error);
      showError('Erro', 'Falha ao carregar o mural de avisos.');
    } finally {
      setLoading(false);
    }
  }, [showError]);
  ...
  const handleFormSuccess = () => {
    loadData();
    success('Sucesso', editingAviso ? 'Aviso atualizado.' : 'Aviso publicado no mural.');
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      success('Excluído', 'Comunicado removido do mural.');
    } catch (error) {
      showError('Erro', 'Não foi possível excluir o aviso.');
    } finally {
  ...
  const filteredAvisos = avisos.filter(a =>
    a.titulo.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
    a.descricao.toLowerCase().includes(debouncedSearch.toLowerCase())
  );
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="mt-2 text-stone-500 font-light max-w-lg">
            Mantenha a comunidade informada sobre ocorrências e eventos.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-24 bg-white rounded-3xl border border-stone-100 shadow-sm flex flex-col items-center">
          <div className="p-4 bg-stone-50 rounded-full mb-4 border border-stone-100"><Megaphone className="w-8 h-8 text-stone-300" /></div>
          <h3 className="text-lg font-medium text-espresso-800">O mural está vazio</h3>
          <p className="text-stone-400 mt-1">
             {debouncedSearch ? 'Tente outra busca.' : 'Crie o primeiro comunicado.'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredAvisos.map((aviso, index) => {
                const isUrgente = aviso.tipo === TipoAviso.URGENTE;
                ...
                return (
                    ...
                    >
                        ...
                        <div>
                            ...
                            <h3 className="text-xl font-semibold text-espresso-900 mb-3 leading-tight line-clamp-2 group-hover:text-clay-700 transition-colors">
                                {aviso.titulo}
                            </h3>
