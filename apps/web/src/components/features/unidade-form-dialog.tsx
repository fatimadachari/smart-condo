...
import { X, Home, Save, Loader2, Layers } from "lucide-react";
import {
  unidadeService,
  CreateUnidadeDto,
  Unidade,
...
export function UnidadeFormDialog({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        {/* Header */}
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                <Layers
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-400 group-focus-within:text-clay-500 transition-colors"
                  strokeWidth={1.5}
                />
                <input
                  {...register("bloco")}
                  placeholder="Ex: A"
                  className="w-full pl-10 pr-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-espresso-900 placeholder-stone-400 focus:outline-none focus:border-clay-300 focus:ring-4 focus:ring-clay-100/50 transition-all"
                />
              </div>
            </div>
          </div>
